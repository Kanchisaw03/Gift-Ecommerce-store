...
export const approveProduct = async (productId, approvalData) => {
  try {
    const response = await axiosInstance.put(`/admin/products/${productId}/approve`, approvalData);
    return response.data;
  } catch (error) {
    throw error.response?.data || { message: 'Failed to approve/reject product' };
  }
};
